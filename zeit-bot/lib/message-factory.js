...
const { checkIntent, randomPick, makeTelMsg } = require("./bot");
const { ROOM_NAMES } = require("../service/room-service");
...
function messageFactory(inputs) {
  ...
  switch (true) {
    ...
    case checkIntent(msgIn, "not", "added", vocab.AND):
      toSend = makeTelMsg(
        "These are the labels not added in the last update: " +
          oldLabels.substring(0, oldLabels.length - 1)
      );
      break;

    case checkIntent(msgIn, "were", "added", vocab.AND):
      toSend = makeTelMsg(
        "These are the labels added in the last update: " +
          labelsAdded.substring(0, labelsAdded.length - 2)
      );
      break;
    ...
    case checkIntent(msgIn, vocab.questionIn, "room"):
      ...
      toSend = makeTelMsg(
        "Hey " + userName + " you are in the " + room + ". Have fun!"
      );
      break;
    ...
    case checkIntent(msgIn, vocab.questionIn, "updater"):
      updateAlive == true
        ? (toSend = makeTelMsg(
            "The playlist updater is currently up and running."
          ))
        : (toSend = makeTelMsg("The playlist updater is currently off."));
      break;

    case checkIntent(msgIn, vocab.playlistIn, "how", vocab.AND):
      toSend = makeTelMsg(
        "you can update your playlist up to once a week with a new track, it must be no more than a week old, if its an album track it can be UPTO a month old"
      );
      break;

    case checkIntent(msgIn, vocab.playlistIn, vocab.questionIn, vocab.AND):
      toSend = makeTelMsg(
        "to add your 1 track a week to the playlist you need to fill out the label submission form here: " +
          LABEL_SUBMISSION_FORM_LINK
      );
      break;
    /*  HATEBOT CODE!
    ...
    case checkIntent(msgIn, "favourite"):
      toSend = makeTelMsg(
        "my favourite track is Messiah by Konflict, my favourite person is Dj Fresh"
      );
      break;
    ...
    case checkIntent(msgIn, "right", "correct"):
      toSend = makeTelMsg(
        "i would always defer to Dj Fresh on matters of judgement"
      );
      break;

    case checkIntent(msgIn, "sell", vocab.questionIn):
      toSend = makeTelMsg(
        "Im not programmed to sell things yet, but let me know what youd be interested in and ill let my humans know"
      );
    ...
    case checkIntent(msgIn, "logo"):
      toSend = makeTelMsg(
        "/Users/lionheart/CODE/APPS/BOTS/LUVBOT/Resources/IluvDNBlogoweb.jpg"
      );
      break;
    ...
    case checkIntent(msgIn, vocab.submissionIn):
      toSend = makeTelMsg(
        "here is the label submission form: " + LABEL_SUBMISSION_FORM_LINK
      );
    ...
    case checkIntent(msgIn, "label", "playlist"):
      toSend = makeTelMsg(
        "you can fill in the label submission form here: " +
          LABEL_SUBMISSION_FORM_LINK
      );
      console.log("Submission Form Requested");
      break;
    ...
    case checkIntent(msgIn, "own", vocab.questionIn):
      toSend = makeTelMsg(
        "when it comes to the playlist, we all own it together"
      );
      break;
    ...
    case checkIntent(msgIn, vocab.questionIn, "you do"):
      toSend = makeTelMsg(
        "I can send you logos, forms or playlist updates, for anything else youre probably better talking to yourself"
      );
      break;
  ...
  switch (true) {
    ...
    case resourceType == "add":
      fs.readFile("/tmp/PlayBoyLabelUrls.txt", "utf8", function (err, data) {
        if (err) {
          toSend = makeTelMsg(
            "I'm afraid there was an Error reading the URL list."
          );
          ...
          if (data.indexOf(msgIn.substr(23)) !== -1) {
            toSend = makeTelMsg(
              "That Url has already been added to the playlist"
            );
          } else {
            tmp = data;
            tmp = tmp + "\n" + msgIn.substr(23);
            fs.writeFile("/tmp/PlayBoyLabelUrls.txt", tmp, function (err) {
              if (err) {
                ...
                console.log("Error adding:" + msgIn.substr(23));
              } else {
                toSend = makeTelMsg(
                  "Url:" + msgIn.substr(23) + " sucessfully added!"
                );
              }
            });
          }
          ...
          if (data.indexOf(destUrl[0]) !== -1) {
            ...
            fs.writeFile("/tmp/PlayBoyLabelUrls.txt", tmp, function (err) {
              if (err) {
                ...
                toSend = makeTelMsg(
                  "Url:" + destUrl[0] + " sucessfully replaced with\n" + repUrl
                );
                console.log(
                  "Url:" + destUrl[0] + " sucessfully replaced with\n" + repUrl
                );
          ...
          if (data.indexOf(msgIn.substr(26)) !== -1) {
            ...
            fs.writeFile("/tmp/PlayBoyLabelUrls.txt", tmp, function (err) {
              if (err) {
                ...
                toSend = makeTelMsg(
                  "Url:" + msgIn.substr(26) + " sucessfully removed!"
                );
                console.log(
                  "Url:" + msgIn.substr(26) + " sucessfully removed!"
                );
    ...
    case resourceType == "form":
      toSend = makeTelMsg(
        "here is the label update form: " + LABEL_UPDATE_FORM_LINK
      );
      break;

    case resourceType == "log":
      toSend = makeTelMsg(
        "The log is no longer available, please speak to Dan Fresh for access"
      );
      break;

    case resourceType == "picture":
      toSend = makeTelMsg(
        "I am a new version in testing i cannot send pictures yet."
      );
      pictureWanted = true;
      break;
    ...
    case resourceType == "update":
      toSend = makeTelMsg(
        "Updating the playlist, report will be available soon."
      );
      res.end("ok");
...
function resourceFactory(inputs) {
  ...
  switch (toSend.type) {
    case "message":
      ...
      axios
        .post("https://api.telegram.org/" + BOT_TOKEN + "/sendMessage", {
    ...
    case "photo":
      axios
        .post("https://api.telegram.org/" + BOT_TOKEN + "/sendPhoto", {
          chat_id: message.chat.id,
          type: "photo",
    ...
    case "status":
      ...
      var time = new Date();
      var chatSize =
        "Hey " + userName + " It's " + time.toUTCString() + " There are ";
      request.get(
        {
          url:
        ...
        function (error1, responseB, response1) {
          chatSize +=
            JSON.parse(response1).result + " People in the Main Room, ";
          request.get(
            {
              url:
                "https://api.telegram.org/" +
            ...
            function (error1, responseB, response1) {
              chatSize +=
                JSON.parse(response1).result +
                " People in the Artist Room and ";
              request.get(
                {
                  url:
                    "https://api.telegram.org/" +
                ...
                function (error1, responseB, response1) {
                  chatSize +=
                    JSON.parse(response1).result +
                    " People in the Label Room. Have a nice day!";
                  axios
                    .post(
...
module.exports = { messageFactory, adminFactory, resourceFactory };
