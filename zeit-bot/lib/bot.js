function checkIntent(string2, anyOfArray, alsoArray, And) {
  ...
  for (let i = 0; i < anyOfArray.length; i++) {
    if (
      string2.indexOf(anyOfArray[i]) !== -1 &&
      typeof anyOfArray == "object"
    ) {
      found = true;
      i = anyOfArray.length;
    } else if (
      typeof anyOfArray == "string" &&
      string2.indexOf(anyOfArray) !== -1
    ) {
      found = true;
    }
  }
  if (And) {
    ...
    if (alsoArray) {
      ...
      for (let j = 0; j < alsoArray.length; j++) {
        if (
          string2.indexOf(alsoArray[j]) !== -1 &&
          typeof alsoArray == "object"
        ) {
          alsoFound++;
        } else if (
          typeof alsoArray == "string" &&
          string2.indexOf(alsoArray) !== -1
        ) {
          alsoFound++;
        }
      }
...
module.exports = { checkIntent, randomPick, makeTelMsg };
