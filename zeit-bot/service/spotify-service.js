...
const { todaysDate } = require("../lib/tools");
...
function playlistFactory(status, track_name, artist_name, uri, album_id) {
  ...
  this.set_status = () => {
    let results = [];
    if (
      todaysDate() > this.release_date &&
      todaysDate() - this.release_date >= 2505600000
    ) {
      ...
      this.oldLabels += `${
        this.label
      }, Added: ${this.added_date
        .toString()
        .substring(0, 10)}, Released: ${this.release_date
        .toString()
        .substring(0, 10)}. `;
      ...
      this.status.text =
        "Date field incorrectly shows " +
        this.release_date.toString().substring(0, 10) +
        ".";
      this.status.colour = colours.FgRed;
      this.to_be_added = true;
      this.to_be_logged = 5;
    ...
    if (
      ...
      (!this.status.text == "Empty" ||
        this.status.text.indexOf("error") !== -1 ||
        this.status.text.indexOf("Error") !== -1)
    ) {
      this.status.colour = colours.FgRed;
      this.to_be_added = false;
    ...
    if (
      (!this.track_name || !this.artist_name || !this.uri || !this.album_id) &&
