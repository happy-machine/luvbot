...
app.post('/new-message', function (req, res) {
    ...
    if (!message||typeof message=="undefined") {
        ...
        //START
        if (typeof message.text!=="undefined") {var msgIn=message.text.toLowerCase()
                ...
                if (id == testRoom || id == adminRoom || id == labelRoom) {
                    ...
                    switch (true) {
                        ...
                        fs.readFile ('/tmp/PlayBoyLabelUrls.txt', 'utf8', function (err, data) {
                            if (err) {throw err; toSend=makeTelMsg('I\'m afraid there was an Error reading the URL list.');
                        ...
                        fs.readFile ('/tmp/PlayBoyLabelUrls.txt', 'utf8', function (err, data) {
                            ...
                            if (err) {throw err; toSend=makeTelMsg('I\'m afraid there was an Error.');console.log("Error reading file"+err)}
                            else { destUrl=msgIn.substr(23).split(' ')
                                    repUrl=destUrl[1]
                                ...
                                if (data.indexOf(destUrl[0])!== -1){
                                    ...
                                    fs.writeFile ('/tmp/PlayBoyLabelUrls.txt', workData, function(err) {
                                        if (err) {
                                            throw err;
                                            toSend=makeTelMsg('Error adding url.');console.log('Error adding:'+repUrl)}
                                                else {
                                                    toSend=makeTelMsg('Url:'+destUrl[0]+' sucessfully replaced with\n'+repUrl);
                                                    console.log('Url:'+destUrl[0]+' sucessfully replaced with\n'+repUrl)
                                                }});
                                    }
                                         else{toSend=makeTelMsg('Url:'+destUrl[0]+' Not found!')
                        ...
                        fs.readFile ('/tmp/PlayBoyLabelUrls.txt', 'utf8', function (err, data) {
                            if (err) {throw err; toSend=makeTelMsg('I\'m afraid there was an Error.')}
                            else { 
                                if (data.indexOf(msgIn.substr(26))!== -1){
                                    ...
                                    fs.writeFile ('/tmp/PlayBoyLabelUrls.txt', workData, function(err) {
                                        if (err) {
                                            throw err;
                                            toSend=makeTelMsg('Error removing url.');console.log('Error removing:'+msgIn.substr(26))}
                                                else {
                                                    toSend=makeTelMsg('Url:'+msgIn.substr(26)+' sucessfully removed!');
                                                    console.log('Url:'+msgIn.substr(26)+' sucessfully removed!')}});
                                    }
                                         else{toSend=makeTelMsg('Url:'+msgIn.substr(26)+' Not found!')
                                                }
                                            }
                                        })
                                        toSend.type="message";
                        ...
                        res.end('ok');updateThePlaylist(null, id);
                    }
                } else {
            ...
            case "photo":
                ...
                axios.post('https://api.telegram.org/bot493535513:AAGVIVZvYqMXwhdyceVQXi6TgXi136aDLXE/sendPhoto', {
                    ...
                    for (ft=0;ft<14;ft++){
                        ...
                        request.get({url:'https://api.telegram.org/bot493535513:AAGVIVZvYqMXwhdyceVQXi6TgXi136aDLXE/getChatMembersCount?chat_id=-1001259716845'}, function (error1, responseB, response1){
                            chatSize+=(JSON.parse(response1).result)+" People in the Main Room, "
                            request.get({url:'https://api.telegram.org/bot493535513:AAGVIVZvYqMXwhdyceVQXi6TgXi136aDLXE/getChatMembersCount?chat_id=-1001138187085'}, function (error1, responseB, response1){
                            chatSize+=(JSON.parse(response1).result)+" People in the Artist Room and "
                            request.get({url:'https://api.telegram.org/bot493535513:AAGVIVZvYqMXwhdyceVQXi6TgXi136aDLXE/getChatMembersCount?chat_id=-1001143247436'}, function (error1, responseB, response1){
...
function updateThePlaylist(resToLambda, room = 459812199){
    ...
    function spotObj (status,track_name,artist_name,uri,album_id) {
        ...
        this.check_within_month = ()=>{return this.release_date + this.added_date},
        this.check_added_within_week =()=>{return this.release_date + this.added_date},
        this.checkthis = (arg)=>{this.uri=arg; return this.uri}
        this.set_status =()=>{
            results=[]
           if (( todaysDate() > this.release_date) && (todaysDate() - this.release_date) >= 2505600000){
            ...
            if (!this.label && this.status.text!=='undefined'){this.status.text="No label Metadata";this.status.colour=colours.FgRed;this.to_be_logged=6}
            if (this.status.text && (!this.status.text=='Empty' || this.status.text.indexOf('error')!==-1 || this.status.text.indexOf('Error')!==-1)){this.status.colour=colours.FgRed;this.to_be_added=false;this.to_be_logged=7}
            if ((!this.track_name || !this.artist_name || !this.uri || !this.album_id) && this.status.text !=='undefined'){this.status.text="MetaData filed incorrectly";this.to_be_logged=8;this.status.colour=colours.FgRed}
                ...
                case 'getTrackNames':
                    ...
                    var throt3 = function (i) {
                        ...
                        return new Promise(function (resolve, reject) {
                            ...
                            that.rpSafe(that.optionsGetTrackNames(i)).then(res => {
                ...
                case 'getUrls':
                    ...
                    var throt2 = function (i, j) {
                        ...
                        return new Promise(function (resolve, reject) {
                            ...
                            that.rpSafe(that.optionsGetUrls(i, j)).then(res => {
                            console.log('url ' + i + ' ' + j)
                    ...
                    var throt = function (spotObj) {
                        ...
                        return new Promise(function (resolve, reject) {
                            ...
                            that.rpSafe(that.optionsGetAlbums(spotObj.album_id)).then(res => {
                ...
                case 'getPlaylistsTracks':
                    ...
                    var throt3 = function (i) {
                        ...
                        return new Promise(function (resolve, reject) {
                            ...
                            that.rpSafe(that.optionsGetPlaylistsTracks(i)).then(res => {
    ...
    function start() {
        ...
        return getPlaylists.multiCall(list)
        ...
        }).then(res => {var calls = [],objToSet=[],callsToMake=[]
           ...
           objToSet[i].status.text='Empty';objToSet[i].status.colour='FgRed';objToSet[i].to_be_logged=8
           if(typeof result!=='undefined' && typeof result.uri!='undefined'){console.log(colours.FgRed,'Playlist '+result.uri+' Empty')
           playlistReport+='Playlist '+result.uri+' Empty\n'
            ...
            var idArray = []
            var dateNow = Math.round(Date.now()/86400000).toString().substr(2, 5)
            for (var scanDate = dateNow; scanDate > 0; scanDate --){
                resultArray.forEach((toBeSorted)=>{
                    //console.log('to be sorted '+JSON.stringify(toBeSorted))
                    
                    if (toBeSorted.to_be_added){ 
                        //console.log('to be added '+JSON.stringify(toBeSorted))
